            ...
            <div class="card-body" id="card_body_peserta">
                ...
                @if (\Auth::user()->role->nama == 'admin')
                    ...
                    <ul class="list-group list-group-flush" id="item-list">
                        @foreach ($posko->poskoPeserta->sortByDesc('id') as $poskoPeserta)
                            @php
                                $nama =
                                    @$poskoPeserta->peserta->nim .
                                    ' - ' .
                                    @$poskoPeserta->peserta->nama .
                                    ' - ' .
                                    @$poskoPeserta->peserta->prodi->alias .
                                    ' - ' .
                                    @$poskoPeserta->peserta->user->jenis_kelamin;
                            @endphp
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <div class="w-75">
                                    <input type="checkbox" class="form-check-input check_peserta_id"
                                        id="peserta_id_{{ $poskoPeserta->id }}" value="{{ $poskoPeserta->id }}">
                                    <label class="form-check-label"
                                        for="peserta_id_{{ $poskoPeserta->id }}">{{ $nama }}</label>
                                </div>
                                <div><button class="btn btn-danger"
                                        onclick="deletePeserta({{ $poskoPeserta->id }}, '{{ $nama }}')"><i
    ...
    @push('script')
        <script>
            ...
            function inputPeserta() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.posko.addPeserta', ['posko' => $posko]) }}",
                    data: {
            ...
            function deletePeserta(poskoPesertaId, nama) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ajaxDeletePeserta(poskoPesertaId);
                    }
                })
            }

            function ajaxDeletePeserta(poskoPesertaId) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.posko.deletePeserta', ['posko' => $posko]) }}",
                    data: {
                    ...
                    dataType: "json",
                    beforeSend: function() {
                        $('#overlay_peserta_' + poskoPesertaId).remove();
                        var div = '<div class="overlay" id="overlay_peserta_' + poskoPesertaId + '">' +
                            '<i class="fas fa-2x fa-sync-alt fa-spin"></i>' +
                            '</div>';
                        $('#card_peserta').append(div);
                    },
