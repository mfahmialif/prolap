    ...
    <div class="container">
        <a href="{{ route('root') }}" class="navbar-brand">
            <img src="{{ asset('/homepage/images/logo-ponpes.png') }}" style="border-radius: 10px" alt="Logo prodi"
                class="brand-image">
            <span class="brand-text font-weight-light">{{ config('app.name') }} - DPL</span>
        </a>
        ...
        <div class="collapse navbar-collapse order-3" id="navbarCollapse">
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a href="{{ route('dpl.dashboard') }}"
                        class="nav-link {{ request()->routeIs('dpl.dashboard') ? ' active-custom' : '' }}">Dashboard</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailPosko', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.posko*') ? ' active-custom' : '' }}
                        {{ request()->routeIs('admin.dpl.detailPosko') ? ' active-custom' : '' }}
                         ">Posko</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailAbsensi', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.absensi*') ? ' active-custom' : '' }}
                        {{ request()->routeIs('admin.dpl.detailAbsensi') ? ' active-custom' : '' }}
                        ">Absensi</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailKegiatanMahasiswa', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.kegiatanMahasiswa*') ? ' active-custom' : '' }}
                        {{ request()->routeIs('admin.dpl.detailKegiatanMahasiswa') ? ' active-custom' : '' }}
                        ">Kegiatan</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailPenugasan', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.penugasan*') ? ' active-custom' : '' }}
                        {{ request()->routeIs('admin.dpl.detailPenugasan') ? ' active-custom' : '' }}
                        ">Penugasan</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailPenilaian', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.penilaian*') ? ' active-custom' : '' }}
                        {{ request()->routeIs('admin.dpl.detailPenilaian') ? ' active-custom' : '' }}
                        ">Penilaian</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.dpl.detailMonitoring', ['dpl' => \Auth::user()->dpl]) }}"
                        class="nav-link
                        {{ request()->routeIs('admin.dpl.detailMonitoring') ? ' active-custom' : '' }}
                        ">Monitoring</a>
                </li>
            </ul>
        </div>
